...
tcpconnect = function(serverip, serverport) {
	...
	gmsocketio_recieve_callback(sock, "ping", callback_39js);
	return sock;
}
...
sendinput = function(sock) {
	gmsocketio_sendinput(sock, buffer);
}
sendping = function(sock) {
	gmsocketio_sendping(sock, buffer);
}
...
gmsocketio_sendinput = function(id, message) {
	if (!sockets[id])
		return;
	sockets[id].emit("newinput", message);

}

gmsocketio_sendping = function(id, message) {
	if (!sockets[id])
		return;
	sockets[id].emit("ping", message);
